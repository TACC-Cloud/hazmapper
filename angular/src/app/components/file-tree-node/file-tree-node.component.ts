...
export class FileTreeNodeComponent implements OnInit, OnDestroy {
  ...
  @Output() clickEvent: EventEmitter<PathTree<Feature>> = new EventEmitter<
    PathTree<Feature>
  >();
  ...
  ngOnInit() {
    this.activeFeatureSub = this.geoDataService.activeFeature.subscribe(
      (next) => {
