...
export class NotificationsService {
  ...
  constructor(private toastr: ToastrService, private envService: EnvService, private http: HttpClient, private authService: AuthService) {
  ...
  getRecent(): void {
    ...
    this.http.get<Array<INotification>>(baseUrl + `?startDate=${then.toISOString()}`).subscribe((notes) => {
  ...
  deleteAllDoneProgress(): void {
    ...
    this.http.delete<Array<IProgressNotification>>(baseUrl).subscribe((notes) => {
      // FIXME: Don't need
      this._progressNotifications.next(notes);
    });
  }
  ...
  deleteProgress(pn: IProgressNotification): void {
    ...
    this.progressNotifications.pipe(take(1)).subscribe((progressList: Array<any>) => {
      progressList = progressList.filter((n) => n.uuid !== pn.uuid);
      this._progressNotifications.next(progressList);
    });

    this.http.delete<Array<IProgressNotification>>(baseUrl + '/' + pn.uuid).subscribe(() => {
  ...
  getProgressByUUID(pn: IProgressNotification): any {
    const baseUrl = this.envService.apiUrl + '/notifications/progress';
    this.http.get<Array<IProgressNotification>>(baseUrl + '/' + pn.uuid).subscribe((note) => {
      return note;
    });
  }
