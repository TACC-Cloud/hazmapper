...
export class ProjectsService {
  ...
  private _currentProjectUser: BehaviorSubject<IProjectUser> =
    new BehaviorSubject<IProjectUser>(null);
  public readonly currentProjectUser: Observable<IProjectUser> =
    this._currentProjectUser.asObservable();

  private _loadingProjectsFailed: BehaviorSubject<boolean> =
    new BehaviorSubject(false);
  public loadingProjectsFailed: Observable<boolean> =
    this._loadingProjectsFailed.asObservable();

  private _loadingActiveProject: BehaviorSubject<boolean> = new BehaviorSubject(
    true
  );
  public loadingActiveProject: Observable<boolean> =
    this._loadingActiveProject.asObservable();

  private _loadingActiveProjectFailed: BehaviorSubject<boolean> =
    new BehaviorSubject(false);
  public loadingActiveProjectFailed: Observable<boolean> =
    this._loadingActiveProjectFailed.asObservable();
  ...
  deleteUserFromProject(proj: Project, uname: string): void {
    this.http
      .delete(this.envService.apiUrl + `/projects/${proj.id}/users/${uname}/`)
      .subscribe(
        (resp) => {
          this.getProjectUsers(proj).subscribe();
        },
  ...
  create(data: ProjectRequest): Observable<Project> {
    return this.http
      .post<Project>(this.envService.apiUrl + `/projects/`, data)
      .pipe(
        tap(
          (proj) => {
            ...
            if (proj.system_id && proj.system_id.startsWith('project')) {
              this.agaveSystemsService.updateProjectMetadata(
                proj,
  ...
  setActiveProjectUUID(uuid: string, usePublicRoute: boolean = false): void {
    ...
    this.http
      .get<Project[]>(this.envService.apiUrl + `/${projectRoute}/?uuid=` + uuid)
      .subscribe(
        (resp) => {
          if (usePublicRoute) {
  ...
  deleteProject(proj: Project): void {
    ...
    this.http
      .delete(this.envService.apiUrl + `/projects/${proj.id}/`)
      .subscribe(
        (resp) => {
          ...
          this._deletingProjects.next(
            this._deletingProjects.value.filter((p) => p.id !== proj.id)
          );
          this.updateProjectsList();
          this.getProjects();
        },
  ...
  updateProject(req: ProjectRequest): void {
    this.http
      .put(this.envService.apiUrl + `/projects/${req.project.id}/`, req)
      .subscribe((resp) => {
  ...
  updateActiveProject(name: string, description: string, isPublic: boolean) {
    ...
    description = description
      ? description
      : this._activeProject.value.description;
    isPublic =
      isPublic !== undefined ? isPublic : this._activeProject.value.public;
    ...
    return this.http
      .put<Project>(
        this.envService.apiUrl + `/projects/${this._activeProject.value.id}/`,
        payload
      )
      .pipe(
        map((updatedProject) => {
          // Update the project list
          const projects = this._projects.value.map((proj) =>
            proj.id === this._activeProject.value.id ? updatedProject : proj
          );
