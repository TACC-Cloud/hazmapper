...
export class JwtInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    if (
      this.envService.jwt &&
      request.url.indexOf(this.envService.apiUrl) > -1 &&
    ...
    if (
      request.url.indexOf(this.envService.streetviewEnv.mapillary.apiUrl) >
        -1 &&
      !(
        request.url.indexOf(this.envService.streetviewEnv.mapillary.tokenUrl) >
        -1
      )
    ) {
    ...
    if (
      ...
      request = request.clone({
        setHeaders: {
          ...
          Authorization:
            'OAuth ' +
            this.envService.streetviewEnv.secrets.mapillary.clientSecret,
        },
      });
    }
